...
/** Lifecycle, keyed state, and Arrow boundary for native regular streaming join. */
public final class NativeRegularJoinBridge {
    ...
    public static long createRocksDb(
        ...
        long handle = createRocksHandle(
                ...
                limit);
        if (handle == 0) {
            throw new IllegalStateException("Native RocksDB regular join returned a null handle");
        }
        return handle;
    }

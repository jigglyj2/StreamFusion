...
/** Lifecycle and Arrow C Data boundary for the native merging SESSION Window TVF. */
public final class NativeSessionWindowTableFunctionBridge {
    ...
    private static final NativeKeyedStateBridge KEYED_STATE_BRIDGE = NativeKeyedStateBridge.of(
            NativeSessionWindowTableFunctionBridge::create,
            NativeSessionWindowTableFunctionBridge::createRocksDb,
            NativeSessionWindowTableFunctionBridge::snapshot,
            NativeSessionWindowTableFunctionBridge::restore,
            NativeSessionWindowTableFunctionBridge::checkpointRocks,
            NativeSessionWindowTableFunctionBridge::importRocksCheckpoint,
            NativeSessionWindowTableFunctionBridge::destroy);

    private NativeSessionWindowTableFunctionBridge() {}
    ...
    public static long create(
        ...
        if (handle == 0) {
            throw new IllegalStateException("Native session Window TVF returned a null handle");
        }
        return handle;
    }
    ...
    public static long createRocksDb(
        ...
        long handle = createRocksHandle(
                ...
                memoryLimit);
        if (handle == 0) {
            throw new IllegalStateException("Native RocksDB session Window TVF returned a null handle");
        }
        return handle;
    }

    public static long process(
            long handle, long inputArray, long inputSchema, long outputArray, long outputSchema, long processingTime) {
        long rows = processArrowBatch(handle, inputArray, inputSchema, outputArray, outputSchema, processingTime);
    ...
    public static long nullRowtimeCount(long handle) {
        return nullRowtimesDropped(handle);
    }
    ...
    private static native long nullRowtimesDropped(long handle);

    ...
    fn process_bounded_input(
        ...
        for row in 0..batch.num_rows() {
            ...
            match self.input_kind(batch, row)? {
                INSERT | UPDATE_AFTER => {
                    let range = if !self.plan.rows_frame && self.plan.preceding_offset.is_some() {
                        range_value(batch.column(self.plan.order_key_index as usize), row)?
                    } else {
                        None
                    };
                    let event_timestamp = range.unwrap_or(i64::MIN);
                    let contributions = row_aggregate_values(&self.calls, batch, row)?;
                    state.rows.entry(order).or_default().push(StoredRow {
                        id: state.next_id,
                        event_timestamp,
                        range_null: range.is_none()
                            && !self.plan.rows_frame
                            && self.plan.preceding_offset.is_some(),
                        payload,
                        contributions,
...
fn recompute_bounded(
    ...
    if rows_frame {
        ...
        for rows in state.rows.values_mut() {
            ...
            if rows[0].range_null {
                let mut null_accumulator = AccumulatorState::new(calls);
                for row in rows.iter() {
                    null_accumulator.apply_values(calls, &row.contributions, true)?;
                }
                let output = null_accumulator.values(calls);
                for row in rows {

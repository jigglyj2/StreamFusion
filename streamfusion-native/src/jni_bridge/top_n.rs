...
use crate::planner::operators::top_n::TopNProcessor;

#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTopNBridge_createHandle<
    'caller,
>(
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            if memory_limit <= 0 {
                return Err(throw(env, "Top-N native memory limit must be positive"));
            }
            ...
            let value = (|| -> datafusion::error::Result<_> {
                TopNProcessor::new(
                    ...
                    HostMemoryReservation::new(broker, "native top-n keyed state"),
                )
            })()
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTopNBridge_createRocksHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            ...
            let memory_limit = usize::try_from(memory_limit)
                .map_err(|_| throw(env, "RocksDB Top-N memory limit must fit usize"))?;
            let broker = Arc::new(JvmMemoryReservationBroker::new(
                env.get_java_vm()?,
                env.new_global_ref(memory_manager)?,
            ));
            let value = (|| -> datafusion::error::Result<_> {
                TopNProcessor::new_rocksdb(
                    &plan,
                    non_negative(max_parallelism, "max parallelism")?,
                    non_negative(first_key_group, "first key group")?,
                    non_negative(last_key_group, "last key group")?,
                    ...
                    memory_limit,
                    HostMemoryReservation::new(broker, "native RocksDB top-n scratch"),
                )
            })()
            .map_err(|error| throw(env, error))?;
            Ok(Box::into_raw(Box::new(value)) as jlong)
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTopNBridge_processBatch<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
        ...
        .with_env(|env| -> jni::errors::Result<_> {
            let rows = (|| -> datafusion::error::Result<_> {
                unsafe {
                    ...
                    let output = processor(handle)?.process_arrow(input, now_millis)?;
                    export_record_batch(
                        output,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTopNBridge_snapshotKeyGroup<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTopNBridge_restoreKeyGroup<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTopNBridge_checkpointRocksHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTopNBridge_importRocksCheckpointHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTopNBridge_destroyHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|_env| -> jni::errors::Result<_> {
            if handle != 0 {
                unsafe { drop(Box::from_raw(handle as *mut TopNProcessor)) };
            }
            Ok(())
        })
        .resolve::<ThrowRuntimeExAndDefault>()
...
fn non_negative(value: jint, description: &str) -> datafusion::error::Result<u32> {
    u32::try_from(value).map_err(|_| {
        datafusion::error::DataFusionError::Execution(format!(
            "top-n {description} must be non-negative"
        ))
    })
}

unsafe fn processor<'a>(handle: jlong) -> datafusion::error::Result<&'a mut TopNProcessor> {
    if handle == 0 {
        return Err(datafusion::error::DataFusionError::Execution(
            "top-n native handle is closed".to_string(),
        ));
    }
    unsafe { (handle as *mut TopNProcessor).as_mut() }.ok_or_else(|| {
        datafusion::error::DataFusionError::Execution("top-n native handle is invalid".to_string())
    })
}

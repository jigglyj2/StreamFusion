...
use crate::planner::operators::interval_join::IntervalJoinProcessor;

#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeIntervalJoinBridge_createHandle<
    'caller,
>(
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            if limit <= 0 {
                return Err(throw(
                    env,
                    "interval join native memory limit must be positive",
                ));
            }
            ...
            let value = (|| -> datafusion::error::Result<_> {
                IntervalJoinProcessor::new(
                    ...
                    HostMemoryReservation::new(broker, "native interval join keyed state"),
                )
            })()
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeIntervalJoinBridge_createRocksHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            ...
            let limit = usize::try_from(limit)
                .map_err(|_| throw(env, "interval join RocksDB limit must fit usize"))?;
            let broker = Arc::new(JvmMemoryReservationBroker::new(
                env.get_java_vm()?,
                env.new_global_ref(manager)?,
            ));
            let value = (|| -> datafusion::error::Result<_> {
                IntervalJoinProcessor::new_rocksdb(
                    &plan,
                    non_negative(max_parallelism, "max parallelism")?,
                    non_negative(first_group, "first key group")?,
                    non_negative(last_group, "last key group")?,
                    ...
                    HostMemoryReservation::new(
                        broker,
                        "native RocksDB interval join scratch and timers",
                    ),
                )
            })()
            .map_err(|error| throw(env, error))?;
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeIntervalJoinBridge_processArrowBatch<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
                ...
                unsafe {
                    ...
                    let output = processor(handle)?.process_arrow(
                        usize::try_from(side).map_err(|_| {
                            DataFusionError::Execution("interval join side is negative".to_string())
                        })?,
                        input,
                        processing_time,
                    )?;
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeIntervalJoinBridge_advanceTime<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    handle: jlong,
    processing_time: jint,
    timestamp: jlong,
    output_array: jlong,
    output_schema: jlong,
) -> jlong {
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeIntervalJoinBridge_nextProcessingTimer<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            unsafe { processor(handle) }
                .map(|value| value.next_processing_timer().unwrap_or(i64::MAX))
                .map_err(|error| throw(env, error))
        })
        .resolve::<ThrowRuntimeExAndDefault>()
}
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeIntervalJoinBridge_nativeStatistics<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeIntervalJoinBridge_snapshotKeyGroup<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeIntervalJoinBridge_restoreKeyGroup<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeIntervalJoinBridge_checkpointRocksHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeIntervalJoinBridge_importRocksCheckpointHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            let limit = usize::try_from(limit)
                .map_err(|_| throw(env, "interval join restore limit must fit usize"))?;
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeIntervalJoinBridge_destroyHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|_env| -> jni::errors::Result<_> {
            if handle != 0 {
                unsafe { drop(Box::from_raw(handle as *mut IntervalJoinProcessor)) };
            }
            Ok(())
        })
        .resolve::<ThrowRuntimeExAndDefault>()
...
fn non_negative(value: jint, description: &str) -> datafusion::error::Result<u32> {
    u32::try_from(value).map_err(|_| {
        DataFusionError::Execution(format!("interval join {description} must be non-negative"))
    })
}

unsafe fn processor<'a>(handle: jlong) -> datafusion::error::Result<&'a mut IntervalJoinProcessor> {
    if handle == 0 {
        return Err(DataFusionError::Execution(
            "interval join native handle is closed".to_string(),
        ));
    }
    unsafe { (handle as *mut IntervalJoinProcessor).as_mut() }.ok_or_else(|| {
        DataFusionError::Execution("interval join native handle is invalid".to_string())
    })
}

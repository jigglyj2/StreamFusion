...
impl AggregateProcessor {
    ...
    fn state_memory(&self) -> crate::memory_pool::HostMemoryReservation {
        match self {
            Self::Group(processor) => processor.state_memory(),
            Self::Global(processor) => processor.state_memory(),
            Self::Incremental(processor) => processor.state_memory(),
        }
    }
    ...
    fn snapshot_key_group(
        &self,
        key_group: u32,
    ) -> datafusion::error::Result<crate::state::SnapshotBytes> {
        match self {
...
fn non_negative(value: jint, description: &str) -> datafusion::error::Result<u32> {
    super::common::non_negative(value, "group aggregate", description)
}

unsafe fn processor<'a>(handle: jlong) -> datafusion::error::Result<&'a mut AggregateProcessor> {
    unsafe { super::common::processor_mut(handle, "group aggregate") }
}

...
use crate::planner::operators::temporal_join::TemporalJoinProcessor;

#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTemporalJoinBridge_createHandle<
    'caller,
>(
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            if limit <= 0 {
                return Err(throw(
                    env,
                    "temporal join native memory limit must be positive",
                ));
            }
            ...
            let value = (|| -> datafusion::error::Result<_> {
                TemporalJoinProcessor::new(
                    ...
                    HostMemoryReservation::new(broker, "native temporal join keyed state"),
                )
            })()
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTemporalJoinBridge_createRocksHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            ...
            let limit = usize::try_from(limit)
                .map_err(|_| throw(env, "temporal join RocksDB limit must fit usize"))?;
            let broker = Arc::new(JvmMemoryReservationBroker::new(
                env.get_java_vm()?,
                env.new_global_ref(manager)?,
            ));
            let value = (|| -> datafusion::error::Result<_> {
                TemporalJoinProcessor::new_rocksdb(
                    &plan,
                    non_negative(max_parallelism, "max parallelism")?,
                    non_negative(first_group, "first key group")?,
                    non_negative(last_group, "last key group")?,
                    ...
                    HostMemoryReservation::new(
                        broker,
                        "native RocksDB temporal join scratch and timers",
                    ),
                )
            })()
            .map_err(|error| throw(env, error))?;
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTemporalJoinBridge_processArrowBatch<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
                ...
                unsafe {
                    ...
                    let output = processor(handle)?.process_arrow(
                        usize::try_from(side).map_err(|_| {
                            DataFusionError::Execution("temporal join side is negative".to_string())
                        })?,
                        input,
                        processing_time,
                    )?;
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTemporalJoinBridge_advanceTime<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTemporalJoinBridge_nextProcessingTimer<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTemporalJoinBridge_nativeStatistics<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTemporalJoinBridge_snapshotKeyGroup<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTemporalJoinBridge_restoreKeyGroup<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTemporalJoinBridge_checkpointRocksHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTemporalJoinBridge_importRocksCheckpointHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeTemporalJoinBridge_destroyHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|_env| -> jni::errors::Result<_> {
            if handle != 0 {
                unsafe { drop(Box::from_raw(handle as *mut TemporalJoinProcessor)) };
            }
            Ok(())
        })
        .resolve::<ThrowRuntimeExAndDefault>()
...
fn non_negative(value: jint, description: &str) -> datafusion::error::Result<u32> {
    u32::try_from(value).map_err(|_| {
        DataFusionError::Execution(format!("temporal join {description} must be non-negative"))
    })
}

unsafe fn processor<'a>(handle: jlong) -> datafusion::error::Result<&'a mut TemporalJoinProcessor> {
    if handle == 0 {
        return Err(DataFusionError::Execution(
            "temporal join native handle is closed".to_string(),
        ));
    }
    unsafe { (handle as *mut TemporalJoinProcessor).as_mut() }.ok_or_else(|| {
        DataFusionError::Execution("temporal join native handle is invalid".to_string())
    })
}

...
>(
    ...
    env.with_env(|env| -> jni::errors::Result<_> {
        ...
        if output == 0 {
            return Err(throw(env, "native IPC output stream must be non-null"));
        }
        let (batches, reservations, rows) = prepare_exchange(
            env,
            &context,
            port,
            plan,
        ...
        let result: Result<_> = (|| {
            let stream = context.start(batches)?;
            unsafe {
                super::common::export_plan_stream(
                    &context,
                    stream,
                    reservations,
...
/// Shared tree/region IPC input edge. The one body-aligned copy remains owned by native
/// batches; only final plan outputs cross C Data back into Java.
pub(super) fn prepare_exchange<'a>(
    env: &mut jni::Env<'a>,
    ...
    Vec<datafusion::execution::memory_pool::MemoryReservation>,
    usize,
)> {
    ...
    let (batches, reservations) = unsafe {
        super::plan_stream::import_inputs(
            context,
            &arrays,
            &schemas,
            ordinal,

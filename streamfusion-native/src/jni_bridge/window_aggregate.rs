...
use crate::planner::operators::window_aggregate::WindowAggregateProcessor;

#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowAggregateBridge_createHandle<
    'caller,
>(
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            if memory_limit <= 0 {
                return Err(throw(
                    env,
                    "window aggregate native memory limit must be positive",
                ));
            }
            ...
            let processor = (|| -> datafusion::error::Result<_> {
                WindowAggregateProcessor::new(
                    ...
                    HostMemoryReservation::new(broker, "native window aggregate keyed state"),
                )
            })()
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowAggregateBridge_createRocksHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            ...
            let memory_limit = usize::try_from(memory_limit)
                .map_err(|_| throw(env, "RocksDB window memory limit must fit usize"))?;
            let broker = Arc::new(JvmMemoryReservationBroker::new(
                env.get_java_vm()?,
                env.new_global_ref(memory_manager)?,
            ));
            let processor = (|| -> datafusion::error::Result<_> {
                WindowAggregateProcessor::new_rocksdb(
                    ...
                    HostMemoryReservation::new(
                        broker,
                        "native RocksDB window aggregate scratch and timers",
                    ),
                )
            })()
            .map_err(|error| throw(env, error))?;
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowAggregateBridge_processArrowBatch<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowAggregateBridge_advanceTime<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowAggregateBridge_nextProcessingTimer<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
                ...
                .and_then(|processor| {
                    i64::try_from(processor.late_records_dropped()).map_err(|_| {
                        datafusion::error::DataFusionError::Execution(
                            "window late record count exceeds Java long".to_string(),
                        )
                    })
                })
                .map_err(|error| throw(env, error))
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowAggregateBridge_nativeStatistics<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowAggregateBridge_snapshotKeyGroup<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowAggregateBridge_restoreKeyGroup<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowAggregateBridge_checkpointRocksHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowAggregateBridge_importRocksCheckpointHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowAggregateBridge_destroyHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|_env| -> jni::errors::Result<_> {
            if handle != 0 {
                unsafe { drop(Box::from_raw(handle as *mut WindowAggregateProcessor)) };
            }
            Ok(())
        })
        .resolve::<ThrowRuntimeExAndDefault>()
...
fn non_negative(value: jint, description: &str) -> datafusion::error::Result<u32> {
    u32::try_from(value).map_err(|_| {
        datafusion::error::DataFusionError::Execution(format!(
            "window aggregate {description} must be non-negative"
        ))
    })
}
...
unsafe fn processor<'a>(
    handle: jlong,
) -> datafusion::error::Result<&'a mut WindowAggregateProcessor> {
    if handle == 0 {
        return Err(datafusion::error::DataFusionError::Execution(
            "window aggregate native handle is closed".to_string(),
        ));
    }
    unsafe { (handle as *mut WindowAggregateProcessor).as_mut() }.ok_or_else(|| {
        datafusion::error::DataFusionError::Execution(
            "window aggregate native handle is invalid".to_string(),
        )
    })
}

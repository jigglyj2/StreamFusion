...
use super::common::export_record_batch;
use crate::memory_pool::{HostMemoryReservation, JvmMemoryReservationBroker};
use crate::planner::operators::regular_join::region::RegularJoinHandle;
...
>(
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            ...
            let mut plan_memory =
                HostMemoryReservation::new(broker.clone(), "regular join JNI plan bytes");
            plan_memory
                .resize(plan.len(env)?)
            ...
            let value = (|| -> datafusion::error::Result<_> {
                RegularJoinHandle::new(
                    ...
                    HostMemoryReservation::new(
                        broker.clone(),
                        "regular join native region control",
                    ),
                    |plan| {
                ...
                RegularJoinHandle::new(
                    ...
                    |plan| {
                        RegularJoinProcessor::new_rocksdb(
                            ...
                            HostMemoryReservation::new(
                                broker,
                                "native RocksDB regular join scratch",
                            ),
                        )
                    },
                )
...
>(
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            let mut values = unsafe { processor(handle) }
                .map(|value| value.statistics())
                .map_err(|error| throw(env, error))?;
            if let Some(region) = unsafe { native_handle(handle) }
                .map_err(|error| throw(env, error))?
                .region
                .as_ref()
            {
                values[2] = region.calc_batches();
            }
        ...
        .with_env(|env| -> jni::errors::Result<_> {
            ...
            (|| -> datafusion::error::Result<()> {
                ...
                for group in first..=last {
                    ...
                    let snapshot = source.snapshot_key_group(group, &target.state_memory())?;
                    target.restore_key_group(group, &snapshot)?;
                }
                Ok(())
            })()
            .map_err(|error| throw(env, error))
...
>(
    ...
    unowned_env
        .with_env(|_env| -> jni::errors::Result<_> {
            if handle != 0 {
                unsafe { drop(Box::from_raw(handle as *mut RegularJoinHandle)) };
            }
            Ok(())
        })
        .resolve::<ThrowRuntimeExAndDefault>()
...
unsafe fn processor<'a>(
    handle: jlong,
) -> datafusion::error::Result<MutexGuard<'a, RegularJoinProcessor>> {
    unsafe { native_handle(handle) }?
        .processor
        .lock()
        .map_err(|_| DataFusionError::Execution("regular join handle is poisoned".into()))
}

pub(super) unsafe fn native_handle<'a>(
    handle: jlong,
) -> datafusion::error::Result<&'a RegularJoinHandle> {
    unsafe { super::common::processor_mut::<RegularJoinHandle>(handle, "regular join") }
        .map(|handle| &*handle)
}

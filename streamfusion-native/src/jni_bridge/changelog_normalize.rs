...
use crate::planner::operators::changelog_normalize::ChangelogNormalizeProcessor;

#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeChangelogNormalizeBridge_createHandle<
    'caller,
>(
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            if memory_limit <= 0 {
                return Err(throw(
                    env,
                    "changelog normalize native memory limit must be positive",
                ));
            }
            ...
            let processor = (|| -> datafusion::error::Result<_> {
                ChangelogNormalizeProcessor::new(
                    ...
                    HostMemoryReservation::new(broker, "native changelog normalize keyed state"),
                )
            })()
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeChangelogNormalizeBridge_createRocksHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            ...
            let memory_limit = usize::try_from(memory_limit).map_err(|_| {
                throw(
                    env,
                    "RocksDB changelog normalize memory limit must fit usize",
                )
            })?;
            let broker = Arc::new(JvmMemoryReservationBroker::new(
                env.get_java_vm()?,
            ...
            let processor = (|| -> datafusion::error::Result<_> {
                ChangelogNormalizeProcessor::new_rocksdb(
                    &plan,
                    non_negative(max_parallelism, "max parallelism")?,
                    non_negative(first_key_group, "first key group")?,
                    non_negative(last_key_group, "last key group")?,
                    ...
                    HostMemoryReservation::new(
                        broker,
                        "native RocksDB changelog normalize scratch",
                    ),
                )
            })()
            .map_err(|error| throw(env, error))?;
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeChangelogNormalizeBridge_processBatch<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
        ...
        .with_env(|env| -> jni::errors::Result<_> {
            let rows = unsafe {
                ...
                let output = processor(handle)
                    .and_then(|processor| processor.process_arrow(input, now_millis))
                    .map_err(|error| throw(env, error))?;
                export_record_batch(
                    output,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeChangelogNormalizeBridge_nativeStatistics<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeChangelogNormalizeBridge_snapshotKeyGroup<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeChangelogNormalizeBridge_restoreKeyGroup<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeChangelogNormalizeBridge_checkpointRocksHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeChangelogNormalizeBridge_importRocksCheckpointHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeChangelogNormalizeBridge_destroyHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|_env| -> jni::errors::Result<_> {
            if handle != 0 {
                unsafe { drop(Box::from_raw(handle as *mut ChangelogNormalizeProcessor)) };
            }
            Ok(())
        })
        .resolve::<ThrowRuntimeExAndDefault>()
...
fn non_negative(value: jint, description: &str) -> datafusion::error::Result<u32> {
    u32::try_from(value).map_err(|_| {
        DataFusionError::Execution(format!(
            "changelog normalize {description} must be non-negative"
        ))
    })
}
...
unsafe fn processor<'a>(
    handle: jlong,
) -> datafusion::error::Result<&'a mut ChangelogNormalizeProcessor> {
    if handle == 0 {
        return Err(DataFusionError::Execution(
            "changelog normalize native handle is closed".to_string(),
        ));
    }
    unsafe { (handle as *mut ChangelogNormalizeProcessor).as_mut() }.ok_or_else(|| {
        DataFusionError::Execution("changelog normalize native handle is invalid".to_string())
    })
}

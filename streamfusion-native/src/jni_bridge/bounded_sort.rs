...
fn non_negative(value: jint, description: &str) -> datafusion::error::Result<u32> {
    u32::try_from(value).map_err(|_| {
        DataFusionError::Execution(format!("bounded sort {description} must be non-negative"))
    })
}
...
unsafe fn processor<'a>(handle: jlong) -> datafusion::error::Result<&'a mut BoundedSortProcessor> {
    if handle == 0 {
        return Err(DataFusionError::Execution(
            "bounded sort native handle is closed".to_string(),
        ));
    }
    unsafe { (handle as *mut BoundedSortProcessor).as_mut() }.ok_or_else(|| {
        DataFusionError::Execution("bounded sort native handle is invalid".to_string())
    })
}

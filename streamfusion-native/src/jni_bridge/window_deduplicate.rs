...
use crate::planner::operators::window_deduplicate::WindowDeduplicateProcessor;

#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowDeduplicateBridge_createHandle<
    'caller,
>(
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            if memory_limit <= 0 {
                return Err(throw(
                    env,
                    "window deduplicate native memory limit must be positive",
                ));
            }
            ...
            let processor = (|| -> datafusion::error::Result<_> {
                WindowDeduplicateProcessor::new(
                    &plan,
                    ...
                    HostMemoryReservation::new(broker, "native window deduplicate keyed state"),
                )
            })()
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowDeduplicateBridge_createRocksHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<_> {
            ...
            let memory_limit = usize::try_from(memory_limit).map_err(|_| {
                throw(
                    env,
                    "RocksDB window deduplicate memory limit must fit usize",
                )
            })?;
            let broker = Arc::new(JvmMemoryReservationBroker::new(
                env.get_java_vm()?,
            ...
            let processor = (|| -> datafusion::error::Result<_> {
                WindowDeduplicateProcessor::new_rocksdb(
                    &plan,
                    non_negative(max_parallelism, "max parallelism")?,
                    non_negative(first_key_group, "first key group")?,
                    non_negative(last_key_group, "last key group")?,
                    ...
                    HostMemoryReservation::new(
                        broker,
                        "native RocksDB window deduplicate scratch and timers",
                    ),
                )
            })()
            .map_err(|error| throw(env, error))?;
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowDeduplicateBridge_processArrowBatch<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowDeduplicateBridge_advanceEventTime<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowDeduplicateBridge_lateRecordsDropped<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
                ...
                .and_then(|processor| {
                    i64::try_from(processor.late_records_dropped()).map_err(|_| {
                        datafusion::error::DataFusionError::Execution(
                            "window deduplicate late-record count exceeds Java long".to_string(),
                        )
                    })
                })
                .map_err(|error| throw(env, error))
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowDeduplicateBridge_nativeStatistics<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowDeduplicateBridge_snapshotKeyGroup<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowDeduplicateBridge_restoreKeyGroup<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowDeduplicateBridge_checkpointRocksHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowDeduplicateBridge_importRocksCheckpointHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
...
#[unsafe(no_mangle)]
pub extern "system" fn Java_tech_streamfusion_nativebridge_NativeWindowDeduplicateBridge_destroyHandle<
    'caller,
>(
    mut unowned_env: EnvUnowned<'caller>,
    _class: JClass<'caller>,
    ...
    unowned_env
        .with_env(|_env| -> jni::errors::Result<_> {
            if handle != 0 {
                unsafe { drop(Box::from_raw(handle as *mut WindowDeduplicateProcessor)) };
            }
            Ok(())
        })
        .resolve::<ThrowRuntimeExAndDefault>()
...
fn non_negative(value: jint, description: &str) -> datafusion::error::Result<u32> {
    u32::try_from(value).map_err(|_| {
        datafusion::error::DataFusionError::Execution(format!(
            "window deduplicate {description} must be non-negative"
        ))
    })
}
...
unsafe fn processor<'a>(
    handle: jlong,
) -> datafusion::error::Result<&'a mut WindowDeduplicateProcessor> {
    if handle == 0 {
        return Err(datafusion::error::DataFusionError::Execution(
            "window deduplicate native handle is closed".to_string(),
        ));
    }
    unsafe { (handle as *mut WindowDeduplicateProcessor).as_mut() }.ok_or_else(|| {
        datafusion::error::DataFusionError::Execution(
            "window deduplicate native handle is invalid".to_string(),
        )
    })
}

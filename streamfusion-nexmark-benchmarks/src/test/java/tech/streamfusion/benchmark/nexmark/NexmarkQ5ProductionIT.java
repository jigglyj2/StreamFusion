...
/** Official Q5 with Flink's default WindowJoin selection and the original RowData generator. */
...
class NexmarkQ5ProductionIT {
    ...
    void bothJoinSelectionsMatchCollectedBytesAndBlackholeCounts(String backend, int parallelism, boolean multiJoin)
        ...
        var flink = LocalRowDataNexmarkBenchmark.run(20_000, "q5", false, backend, parallelism);
        assertThat(flink.completed()).isTrue();
        assertThat(flink.nativePlanBatches()).isZero();
        var accelerated = LocalRowDataNexmarkBenchmark.run(20_000, "q5", true, backend, parallelism);
        ...
        StreamFusionPlannerFactory.resetMetrics();
        long baselineRows = NexmarkRowDataJob.runBlackhole(20_000, "q5", false, backend, parallelism, false);
        assertThat(StreamFusionPlannerFactory.nativePlanBatchCount()).isZero();
        StreamFusionPlannerFactory.resetMetrics();
        long nativeRows = NexmarkRowDataJob.runBlackhole(20_000, "q5", true, backend, parallelism, false);

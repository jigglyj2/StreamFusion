...
class NexmarkQ16ProductionIT {
    ...
    void catalogSelectIsTheUnchangedUpstreamQuery() throws Exception {
        try (var input = getClass().getResourceAsStream("/queries/q16.sql")) {
            ...
            var select = original.substring(original.indexOf("SELECT\n", original.indexOf("INSERT INTO")))
                    .replace(";", "")
                    .trim();
            assertThat(NexmarkRowDataQueryCatalog.load("q16").trim()).isEqualTo(select);
            assertThat(NexmarkRowDataQueryCatalog.sinkColumns("q16")).doesNotContain("PRIMARY KEY");
        }
    }

    @ParameterizedTest
    @CsvSource({"hashmap,1", "rocksdb,1", "hashmap,4", "rocksdb,4"})
    void channelStatisticsAndBlackholeCountsMatchFlink(String backend, int parallelism) throws Exception {
        var flink = LocalRowDataNexmarkBenchmark.run(50_000, "q16", false, backend, parallelism);
        assertThat(flink.completed()).isTrue();
        assertThat(StreamFusionPlannerFactory.nativePlanBatchCount()).isZero();
        var nativeResult = LocalRowDataNexmarkBenchmark.run(50_000, "q16", true, backend, parallelism);
        ...
        // differently, producing different transient counts for the same final channel statistics.
        ...
        StreamFusionPlannerFactory.resetMetrics();
        long flinkBlackhole = NexmarkRowDataJob.runBlackhole(50_000, "q16", false, backend, parallelism, false);
        assertThat(StreamFusionPlannerFactory.nativePlanBatchCount()).isZero();
        StreamFusionPlannerFactory.resetMetrics();
        long nativeBlackhole = NexmarkRowDataJob.runBlackhole(50_000, "q16", true, backend, parallelism, false);

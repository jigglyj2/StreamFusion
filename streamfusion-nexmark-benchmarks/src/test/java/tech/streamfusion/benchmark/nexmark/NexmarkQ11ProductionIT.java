...
/** Opt-in official Q11 SESSION parity through ordinary planner selection and the Nexmark RowData source. */
@ResourceLock("streamfusion-planner-property")
class NexmarkQ11ProductionIT {
    @AfterEach
    void clearPlanner() {
    ...
    void sessionCountsPreserveCompleteNexmarkResults(String backend) throws Exception {
        for (int parallelism : List.of(1, 4)) {
            var flink = LocalRowDataNexmarkBenchmark.run(10_000, "q11", false, backend, parallelism);
            assertThat(flink.completed()).isTrue();
            assertThat(flink.outputRows()).isPositive();
            var accelerated = LocalRowDataNexmarkBenchmark.run(10_000, "q11", true, backend, parallelism);

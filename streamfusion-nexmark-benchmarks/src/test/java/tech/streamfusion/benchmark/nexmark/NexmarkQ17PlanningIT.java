...
/** Original auction statistics SQL through ordinary planner admission and the blackhole sink. */
@ResourceLock("streamfusion-planner-property")
class NexmarkQ17PlanningIT {
    @AfterEach
    void clearPlanner() {
    ...
    void originalQ17EstablishesWholePlanAdmissionOnBothBackends(boolean selected, String backend) throws Exception {
        if (selected)
            System.setProperty(
        ...
        try (var input = getClass().getResourceAsStream("/queries/q17.sql")) {
            ...
            sql = new String(input.readAllBytes(), StandardCharsets.UTF_8).replace("nexmark_q17", "nexmark_output");
        }

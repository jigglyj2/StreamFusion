...
/** Official Q7 tumbling maximum and timestamp join under both Flink join selections. */
...
class NexmarkQ7ProductionIT {
    ...
    void bothJoinSelectionsMatchCollectedBytesAndBlackholeCounts(String backend, int parallelism, boolean multiJoin)
        ...
        StreamFusionPlannerFactory.resetMetrics();
        long baselineRows = NexmarkRowDataJob.runBlackhole(10_000, "q7", false, backend, parallelism, false);
        assertThat(StreamFusionPlannerFactory.nativePlanBatchCount()).isZero();
        StreamFusionPlannerFactory.resetMetrics();
        long nativeRows = NexmarkRowDataJob.runBlackhole(10_000, "q7", true, backend, parallelism, false);

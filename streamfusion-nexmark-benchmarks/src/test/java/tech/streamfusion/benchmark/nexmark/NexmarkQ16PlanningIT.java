...
/** Original channel statistics SQL through ordinary planner admission and the blackhole sink. */
@ResourceLock("streamfusion-planner-property")
class NexmarkQ16PlanningIT {
    @AfterEach
    void clearPlanner() {
    ...
    void originalQ16EstablishesWholePlanAdmissionOnBothBackends(boolean selected, String backend) throws Exception {
        if (selected)
            System.setProperty(
        ...
        try (var input = getClass().getResourceAsStream("/queries/q16.sql")) {
            ...
            sql = new String(input.readAllBytes(), StandardCharsets.UTF_8).replace("nexmark_q16", "nexmark_output");
        }

...
class NexmarkQ17ProductionIT {
    ...
    void catalogSelectIsTheUnchangedUpstreamQuery() throws Exception {
        try (var input = getClass().getResourceAsStream("/queries/q17.sql")) {
            ...
            var select = original.substring(original.indexOf("SELECT\n", original.indexOf("INSERT INTO")))
                    .replace(";", "")
                    .trim();
            assertThat(NexmarkRowDataQueryCatalog.load("q17").trim()).isEqualTo(select);
            assertThat(NexmarkRowDataQueryCatalog.sinkColumns("q17")).doesNotContain("PRIMARY KEY");
        }
    }

    @ParameterizedTest
    @CsvSource({"hashmap,1", "rocksdb,1", "hashmap,4", "rocksdb,4"})
    void auctionStatisticsAndBlackholeCountsMatchFlink(String backend, int parallelism) throws Exception {
        var flink = LocalRowDataNexmarkBenchmark.run(50_000, "q17", false, backend, parallelism);
        assertThat(flink.completed()).isTrue();
        assertThat(StreamFusionPlannerFactory.nativePlanBatchCount()).isZero();
        var nativeResult = LocalRowDataNexmarkBenchmark.run(50_000, "q17", true, backend, parallelism);
        ...
        // differently, producing different transient counts for the same final auction statistics.
        ...
        StreamFusionPlannerFactory.resetMetrics();
        long flinkBlackhole = NexmarkRowDataJob.runBlackhole(50_000, "q17", false, backend, parallelism, false);
        assertThat(StreamFusionPlannerFactory.nativePlanBatchCount()).isZero();
        StreamFusionPlannerFactory.resetMetrics();
        long nativeBlackhole = NexmarkRowDataJob.runBlackhole(50_000, "q17", true, backend, parallelism, false);

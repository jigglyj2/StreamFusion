...
class LocalRowDataNexmarkBenchmarkIT {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"hashmap", "rocksdb"})
    void boundedOnePhaseWindowAggregatesMatchFlinkWithoutInventingALocalStage(String backend) throws Exception {
        System.setProperty("streamfusion.nexmark.batch-mode", "true");
        System.setProperty("streamfusion.nexmark.aggregate-phase", "ONE_PHASE");
        for (String query :
            ...
            assertThat(streamFusion.outputSha256()).isEqualTo(flink.outputSha256());
            assertThat(StreamFusionPlanningDiagnostics.explain()).contains("Accelerated: yes");
            assertThat(streamFusion.nativeLocalWindowAggregateBatches()).isZero();
            assertThat(streamFusion.nativeWindowAggregateBatches()).isGreaterThan(0);
            assertThat(streamFusion.nativeBoundedSortBatches()).isZero();
        }
        ...
        assertThat(distributedStreamFusion.outputSha256()).isEqualTo(distributedFlink.outputSha256());
        assertThat(StreamFusionPlanningDiagnostics.explain()).contains("Accelerated: yes");
        assertThat(distributedStreamFusion.nativeLocalWindowAggregateBatches()).isZero();
        assertThat(distributedStreamFusion.nativeWindowAggregateBatches()).isGreaterThan(0);
        assertThat(distributedStreamFusion.nativeBoundedSortBatches()).isZero();
    }

    @ParameterizedTest
    @ValueSource(strings = {"hashmap", "rocksdb"})
    void boundedQ20JoinMatchesFlinkThroughTheRowDataNexmarkBoundary(String backend) throws Exception {
        System.setProperty("streamfusion.nexmark.batch-mode", "true");
        LocalRowDataNexmarkBenchmark.RunResult flink =
                LocalRowDataNexmarkBenchmark.run(2_000, "q20", false, backend, 1);
        LocalRowDataNexmarkBenchmark.RunResult streamFusion =
                LocalRowDataNexmarkBenchmark.run(2_000, "q20", true, backend, 1);

        assertThat(streamFusion.completed()).as(backend).isTrue();
        assertThat(streamFusion.debugRows()).as(backend).containsExactlyElementsOf(flink.debugRows());
        assertThat(streamFusion.outputRows()).as(backend).isEqualTo(flink.outputRows());
        assertThat(streamFusion.outputSha256()).as(backend).isEqualTo(flink.outputSha256());
        assertThat(StreamFusionPlanningDiagnostics.explain()).as(backend).contains("Accelerated: yes");
        assertThat(streamFusion.nativeRegularJoinBatches()).as(backend).isGreaterThan(0);
        assertThat(streamFusion.nativeCalcBatches()).as(backend).isGreaterThan(0);
    }
    ...
    @ParameterizedTest
    @ValueSource(strings = {"hashmap", "rocksdb"})
    void boundedMatchRecognizeMatchesFlinkOnBothStateBackends(String backend) throws Exception {
        System.setProperty("streamfusion.nexmark.batch-mode", "true");
        LocalRowDataNexmarkBenchmark.RunResult flink =
                LocalRowDataNexmarkBenchmark.run(2_000, "bounded-match-recognize", false, backend, 4);
        LocalRowDataNexmarkBenchmark.RunResult streamFusion =
                LocalRowDataNexmarkBenchmark.run(2_000, "bounded-match-recognize", true, backend, 4);
    ...
    @ParameterizedTest
    @ValueSource(strings = {"hashmap", "rocksdb"})
    void boundedSortMergeJoinMatchesFlinkOnBothConfiguredBackends(String backend) throws Exception {
        System.setProperty("streamfusion.nexmark.batch-mode", "true");
        LocalRowDataNexmarkBenchmark.RunResult flink =
                LocalRowDataNexmarkBenchmark.run(2_000, "bounded-sort-merge-join", false, backend, 4);
        LocalRowDataNexmarkBenchmark.RunResult streamFusion =
                LocalRowDataNexmarkBenchmark.run(2_000, "bounded-sort-merge-join", true, backend, 4);

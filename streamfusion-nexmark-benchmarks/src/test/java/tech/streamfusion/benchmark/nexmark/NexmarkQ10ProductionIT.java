...
/** Opt-in official Q10 SELECT parity through ordinary planner selection and the Nexmark RowData source. */
@ResourceLock("streamfusion-planner-property")
class NexmarkQ10ProductionIT {
    @AfterEach
    void clearPlanner() {
    ...
    void numericPartitionLabelsPreserveCompleteNexmarkSelectResults(String backend) throws Exception {
        for (int parallelism : List.of(1, 4)) {
            var flink = LocalRowDataNexmarkBenchmark.run(10_000, "q10", false, backend, parallelism);
            assertThat(flink.completed()).isTrue();
            assertThat(flink.outputRows()).isPositive();
            var accelerated = LocalRowDataNexmarkBenchmark.run(10_000, "q10", true, backend, parallelism);
            ...
            assertThat(accelerated.nativeCalcBatches()).isPositive();
        }
    }
}

...
/** Keep the original UDF and SQL while establishing the next production blockers. */
@ResourceLock("streamfusion-planner-property")
class NexmarkQ14PlanningIT {
    @AfterEach
    void clearPlanner() {
    ...
    void originalQ14ExecutesWithItsJavaUdfAndPreciseWholePlanFallback(boolean selected, String backend)
            throws Exception {
        if (selected)
            System.setProperty(
        ...
        String sql;
        try (var input = getClass().getResourceAsStream("/queries/q14.sql")) {
            assertThat(input).isNotNull();
            sql = new String(input.readAllBytes(), StandardCharsets.UTF_8).replace("nexmark_q14", "nexmark_output");
        }
        var statements = sql.split(";");
        assertThat(statements).hasSize(3);
        tables.executeSql(statements[0]);
        tables.executeSql(statements[1]);
        var plan = tables.explainSql(statements[2]);
        assertThat(plan).contains("count_char", "Calc");
        if (selected) {

...
class NexmarkQ23ProductionIT {
    ...
    void sellerBidsAndBlackholeCountsMatchFlink(String backend, int parallelism) throws Exception {
        // The benchmark historically enables MultiJoin; use Flink's default physical plan.
        System.setProperty(OptimizerConfigOptions.TABLE_OPTIMIZER_MULTI_JOIN_ENABLED.key(), "false");
        var flink = LocalRowDataNexmarkBenchmark.run(100_000, "q23", false, backend, parallelism);
        assertThat(flink.completed()).isTrue();
        assertThat(StreamFusionPlannerFactory.nativePlanBatchCount()).isZero();
        var nativeResult = LocalRowDataNexmarkBenchmark.run(100_000, "q23", true, backend, parallelism);
        ...
        StreamFusionPlannerFactory.resetMetrics();
        long flinkBlackhole = NexmarkRowDataJob.runBlackhole(100_000, "q23", false, backend, parallelism, false);
        assertThat(StreamFusionPlannerFactory.nativePlanBatchCount()).isZero();
        StreamFusionPlannerFactory.resetMetrics();
        long nativeBlackhole = NexmarkRowDataJob.runBlackhole(100_000, "q23", true, backend, parallelism, false);

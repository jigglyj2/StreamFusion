...
| Group Aggregate | Flink `GroupAggFunction`, `MiniBatchGroupAggFunction`, and local/global mini-batch aggregate operators | The immediate shape has no additional Flink operator counter. One- and two-phase count-triggered mini-batch stages publish Flink's `bundleSize` and `bundleRatio` gauges from their native pending bundles. Standard IO counters are corrected from Arrow batches to logical rows; native state-call counts come from the processor rather than being inferred from Arrow batches, and changelog/checkpoint diagnostics are additive. |

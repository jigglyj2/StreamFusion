...
| Window Aggregate | Flink window aggregate functions and trigger operators | Each preserved local/global phase publishes its own logical-record IO counters and managed-memory usage. The stateful global phase additionally publishes native state, changelog, late-row, event/processing timer, pending-timer, checkpoint, and restore diagnostics at the corresponding state or timer decision. |

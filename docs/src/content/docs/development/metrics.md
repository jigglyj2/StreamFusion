...
| Group Aggregate | Flink `GroupAggFunction` / `MiniBatchGroupAggFunction` | The immediate shape has no additional Flink operator counter. Count-triggered mini-batch aggregation publishes Flink's `bundleSize` and `bundleRatio` gauges from the native pending bundle. Standard IO counters are corrected from Arrow batches to logical rows; native state/changelog/checkpoint diagnostics are additive. |

...
public final class StreamFusionExecWindowTableFunction extends CommonExecWindowTableFunction
    ...
    protected Transformation<RowData> translateToPlanInternal(PlannerBase planner, ExecNodeConfig config) {
        ...
        RowDataKeySelector selector = null;
        if (streamFusionStrategy.getWindow() instanceof SessionWindowSpec) {
            selector = KeySelectorUtil.getRowDataSelector(
                    planner.getFlinkContext().getClassLoader(),
                    ((SessionWindowSpec) streamFusionStrategy.getWindow()).getPartitionKeyIndices(),

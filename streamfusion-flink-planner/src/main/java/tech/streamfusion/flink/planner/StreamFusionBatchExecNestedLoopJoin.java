...
public final class StreamFusionBatchExecNestedLoopJoin extends ExecNodeBase<RowData> implements BatchExecNode<RowData> {
    ...
    public StreamFusionBatchExecNestedLoopJoin(
        ...
        super(
                ...
                new ExecNodeContext("streamfusion-batch-exec-nested-loop-join_1"),
                config,
    ...
    private Transformation<RowData> translateInternal(
        ...
        RowDataKeySelector leftSelector = KeySelectorUtil.getRowDataSelector(
                planner.getFlinkContext().getClassLoader(), new int[0], InternalTypeInfo.of(leftType));
        ...
        try {
            ...
            if (result == null) {
                throw new IllegalStateException("A selected StreamFusion bounded nested-loop join failed translation");
            }
            return result;
        } catch (ClassNotFoundException | NoSuchMethodException | IllegalAccessException failure) {
            throw new IllegalStateException("Could not invoke the native bounded nested-loop join", failure);
        } catch (InvocationTargetException failure) {
            throw new IllegalStateException("Native bounded nested-loop join translation failed", failure.getCause());
        }
    }
}

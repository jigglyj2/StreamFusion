...
/** Distinct StreamFusion physical node for a final, one-phase bounded sort aggregate. */
public final class StreamFusionBatchExecSortAggregate extends ExecNodeBase<RowData> implements BatchExecNode<RowData> {
    ...
    public StreamFusionBatchExecSortAggregate(
        ...
        super(
                ...
                new ExecNodeContext("streamfusion-batch-exec-sort-aggregate_1"),
                config,
    ...
    protected Transformation<RowData> translateToPlanInternal(PlannerBase planner, ExecNodeConfig config) {
        ...
        try {
            ...
            Transformation<RowData> result = (Transformation<RowData>) method.invoke(
                    ...
                    selector);
            if (result == null) {
                throw new IllegalStateException("A selected bounded StreamFusion sort aggregate failed translation");
            }
            return result;
        } catch (ClassNotFoundException | NoSuchMethodException | IllegalAccessException failure) {
            throw new IllegalStateException("Could not invoke the native bounded sort aggregate", failure);
        } catch (InvocationTargetException failure) {
            throw new IllegalStateException("Native bounded sort aggregate translation failed", failure.getCause());
        }
    }
}

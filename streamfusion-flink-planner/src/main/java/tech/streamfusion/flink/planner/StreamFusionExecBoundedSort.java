...
/** Distinct StreamFusion physical node for bounded global full sort. */
public final class StreamFusionExecBoundedSort extends ExecNodeBase<RowData> implements StreamExecNode<RowData> {
    private static final String TRANSLATOR_CLASS = "tech.streamfusion.flink.sort.StreamFusionBoundedSortTranslator";

    private final SortSpec sortSpec;

    public StreamFusionExecBoundedSort(
        ...
        super(
                ...
                new ExecNodeContext("streamfusion-exec-bounded-sort_1"),
                config,
    ...
    protected Transformation<RowData> translateToPlanInternal(PlannerBase planner, ExecNodeConfig config) {
        ...
        try {
            ...
            if (result == null) {
                throw new IllegalStateException("A selected StreamFusion bounded sort failed translation");
            }
            return result;
        } catch (ClassNotFoundException | NoSuchMethodException | IllegalAccessException failure) {
            throw new IllegalStateException("Could not invoke the StreamFusion bounded sort runtime", failure);
        } catch (InvocationTargetException failure) {
            throw new IllegalStateException("StreamFusion bounded sort translation failed", failure.getCause());
        }
    }
}

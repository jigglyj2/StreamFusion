...
import tech.streamfusion.flink.planner.window.StreamFusionWindowDeduplicatePlan;
import tech.streamfusion.flink.state.StreamFusionStateBackend;

class StreamFusionArrowWindowDeduplicateOperatorTest {
    ...
    private static final RowType INPUT_TYPE = RowType.of(
            ...
            new String[] {"key", "order_time", "window_end", "payload"});
    ...
    void exposesFlinkWindowMetricsWithLogicalRecordSemantics() throws Exception {
        ...
        byte[] plan = StreamFusionWindowDeduplicatePlan.create(INPUT_TYPE, new int[] {0}, 1, 2, true, "UTC");
        StreamFusionArrowWindowDeduplicateOperator operator =
                new StreamFusionArrowWindowDeduplicateOperator(INPUT_TYPE, new int[] {0}, plan, selector);
        try (RootAllocator inputs = new RootAllocator(64L << 20);
                MockEnvironment environment = new MockEnvironmentBuilder()
                        .setTaskName("Window deduplicate metric parity")
            ...
            // The task/operator chain counts one physical Arrow transport record around the
            // operator. The bare test harness does not, so model those two framework updates.
            ((Counter) metrics.get("numRecordsIn")).inc();
            process(harness, inputs, row(7, 1_000, "winner", RowKind.INSERT));
            ...
            ((Counter) metrics.get("numRecordsIn")).inc();
            process(harness, inputs, row(8, 1_000, "late", RowKind.INSERT));
                ...
                for (boolean targetRocks : new boolean[] {false, true}) {
                    for (SnapshotKind kind : SnapshotKind.values()) {
                        ...
                        try (Harness source = harness(1, 0, null, sourceRocks)) {
                            process(source.harness, inputs, row(7, 1_000, "old", RowKind.INSERT));
                            process(source.harness, inputs, row(7, 2_000, "new", RowKind.INSERT));
                            snapshot = snapshot(source.harness, kind);
                        }
                        try (Harness target = harness(1, 0, snapshot, targetRocks)) {
                            process(target.harness, inputs, row(7, 2_000, "new", RowKind.DELETE));
            ...
            for (boolean rocks : new boolean[] {false, true}) {
                ...
                for (int subtask = 0; subtask < 2; subtask++) {
                    ...
                    try (Harness scaled = harness(2, subtask, assigned, rocks)) {
                        ...
                        outputRows += takeOutputRows(scaled.harness);
                        long key = rows.get(subtask).getLong(0);
                        process(scaled.harness, inputs, row(key, 11_000, 20_000, "second-" + key, RowKind.INSERT));
    ...
    void rocksCheckpointsReuseSstsAndRestoreArrowRowState() throws Exception {
        try (RootAllocator inputs = new RootAllocator(64L << 20)) {
            ...
            try (Harness source = harness(1, 0, null, true)) {
                process(source.harness, inputs, row(7, 1_000, "winner", RowKind.INSERT));
    ...
    private static RowDataKeySelector selector() {
        return KeySelectorUtil.getRowDataSelector(
                StreamFusionArrowWindowDeduplicateOperatorTest.class.getClassLoader(),
                new int[] {0},
    ...
    private static Map<Integer, GenericRowData> rowsForOwners(RowDataKeySelector selector, int parallelism)
        ...
        for (long key = 0; rows.size() < parallelism; key++) {
            GenericRowData row = row(key, 1_000, "owner-" + key, RowKind.INSERT);
    ...
    private static GenericRowData row(long key, long orderTime, String payload, RowKind kind) {
        return row(key, orderTime, 10_000, payload, kind);
    }

    private static GenericRowData row(long key, long orderTime, long windowEnd, String payload, RowKind kind) {
        GenericRowData row = GenericRowData.of(
                key,
                TimestampData.fromEpochMillis(orderTime),
                TimestampData.fromEpochMillis(windowEnd),

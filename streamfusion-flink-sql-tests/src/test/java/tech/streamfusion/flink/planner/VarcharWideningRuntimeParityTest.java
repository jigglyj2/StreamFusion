...
/** Compares Flink-generated widening casts, including its declared-width trust semantics. */
class VarcharWideningRuntimeParityTest {
    private static final RowType INPUT = RowType.of(new VarCharType(9));

    @ParameterizedTest
    @ValueSource(booleans = {false, true})
    void wideningPreservesChangelogControlsAndMetrics(boolean rocks) throws Exception {
        var types = new FlinkTypeFactory(getClass().getClassLoader(), FlinkTypeSystem.INSTANCE);
        var rex = new RexBuilder(types);
        var value = rex.makeInputRef(types.createFieldTypeFromLogicalType(INPUT.getTypeAt(0)), 0);
        var projections = new ArrayList<RexNode>();
        projections.add(value);
        for (int width : new int[] {9, 20, 64, VarCharType.MAX_LENGTH}) {
            projections.add(rex.makeAbstractCast(types.createFieldTypeFromLogicalType(new VarCharType(width)), value));
        }
        ...
        try (var oracle = oracle(output, projections);
            ...
            for (int count : new int[] {0, 1, 31, 3001}) {
                ...
                for (int index = 0; index < count; index++) {
                    String text = index < edges.length ? edges[index] : "a界😀 ".repeat(random.nextInt(17));
                    var row = GenericRowData.of(text == null ? null : StringData.fromString(text));

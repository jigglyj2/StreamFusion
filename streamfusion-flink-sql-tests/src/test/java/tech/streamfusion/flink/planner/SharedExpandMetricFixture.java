...
final class SharedExpandMetricFixture {
    static final RowType TYPE = RowType.of(new IntType());
    ...
    private final List<List<RexNode>> projects = List.of(
            identity,
            List.of(rex.makeNullLiteral(id.getType())),
            List.of(rex.makeCall(SqlStdOperatorTable.PLUS, id, rex.makeExactLiteral(BigDecimal.ONE))));
    ...
    static String uid(int stage) {
        return "shared-expand-metric-stage-" + stage;
    }
    ...
    private RexNode condition(int stage) {
        return rex.makeCall(
                stage == 0 ? SqlStdOperatorTable.GREATER_THAN_OR_EQUAL : SqlStdOperatorTable.LESS_THAN_OR_EQUAL,
                id,
                rex.makeExactLiteral(BigDecimal.valueOf(stage == 0 ? 0 : 3)));
    }
    ...
    byte[] plan() {
        ...
        for (int stage = 0; stage < 3; stage++) {
            byte[] fragment = stage == 1
                    ? StreamFusionExpandTranslator.createStagePlan(TYPE, TYPE, new ArrayList<>(projects))
                    : StreamFusionCalcTranslator.createStagePlan(TYPE, TYPE, identity, condition(stage));
            fragments.add(
    ...
    Oracle oracle(int stage) throws Exception {
        ...
        String generatedName = "ExpandMetricStage" + stage;
        var factory = stage == 1
                ? ExpandCodeGenerator.generateExpandOperator(generator, TYPE, TYPE, projects, true, generatedName)

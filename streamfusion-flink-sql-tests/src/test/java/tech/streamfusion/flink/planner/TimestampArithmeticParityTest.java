...
/** Compares actual Flink-generated arithmetic, including the full internal millisecond range. */
class TimestampArithmeticParityTest {
    private static final RowType INPUT = RowType.of(new TimestampType(3));

    @Test
    void generatedDayTimeArithmeticPreservesOverflowNullsChangelogAndMetrics() throws Exception {
        var types = new FlinkTypeFactory(getClass().getClassLoader(), FlinkTypeSystem.INSTANCE);
        var rex = new RexBuilder(types);
        var timestamp = rex.makeInputRef(types.createFieldTypeFromLogicalType(INPUT.getTypeAt(0)), 0);
        var projections = new ArrayList<RexNode>();
        projections.add(timestamp);
        for (long offset : new long[] {-86_400_001, -10_000, 0, 10_000, 86_400_001, Long.MIN_VALUE, Long.MAX_VALUE}) {
            var interval = rex.makeIntervalLiteral(
                    BigDecimal.valueOf(offset),
                    new SqlIntervalQualifier(TimeUnit.DAY, TimeUnit.SECOND, SqlParserPos.ZERO));
            for (var op : List.of(SqlStdOperatorTable.PLUS, SqlStdOperatorTable.MINUS))
                projections.add(rex.makeCall(op, timestamp, interval));
        }
        var output = RowType.of(
                projections.stream().map(ignored -> new TimestampType(3)).toArray(LogicalType[]::new));
        ...
        try (var oracle = oracle(output, projections);
                var nativePlan = new KeyedNativeMetricHarness(false, plan, List.of(INPUT), output, List.of());
                var allocator = new RootAllocator(64L << 20)) {
            var expected = new DataOutputSerializer(128);
            var random = new Random(197);
            long[] edges = {Long.MIN_VALUE, Long.MAX_VALUE, -86_400_001, -1, 0, 1, 1_600_000_000_000L};
            for (int count : new int[] {0, 1, 31, 3001}) {
                ...
                for (int index = 0; index < count; index++) {
                    long millis = index < edges.length ? edges[index] : random.nextLong();
                    var row = GenericRowData.of(index % 17 == 16 ? null : TimestampData.fromEpochMillis(millis));

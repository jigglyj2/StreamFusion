...
class SharedDeduplicateRecoveryTest {
    ...
    void checkpointsBackendSwitchAndRescalingPreserveEveryDeduplicateTransition(
            boolean rocks, int mode, boolean ascending) throws Exception {
        for (int seed : List.of(3, 19, 71)) {
            var fixture = new SharedDeduplicateMetricFixture(true, !ascending, true, true);
    ...
    private static KeyedNativeMetricHarness region(
        ...
        return new KeyedNativeMetricHarness(
                rocks, factory, 1, SharedDeduplicateMetricFixture.TYPE, state, parallelism, subtask);
    }

    private static byte[] exchange(SharedDeduplicateMetricFixture fixture, int parallelism) {
        return NativeExchangePlanSerializer.hash(
                SharedDeduplicateMetricFixture.TYPE, new int[] {0}, 16, parallelism, true);
    }
    ...
    private static void compare(
        ...
        for (int key = 0; key < 65; key++) {
            ...
            long[] timestamps = {Long.MIN_VALUE, 0, phase == 3 ? Long.MAX_VALUE : phase * 1000L + key};
            var expected = new DataOutputSerializer(128);
            for (int arrival = 0; arrival < 3; arrival++) {
                var row = GenericRowData.of(
                        key == 64 ? null : StringData.fromString("é-" + key),
                        TimestampData.fromEpochMillis(random.nextInt(4)),
                        random.nextInt(5) == 0 ? null : (long) random.nextInt(12) - 2);
                rows.add(row);
            ...
            try (var batch = ArrowRowDataBatch.transpose(rows, SharedDeduplicateMetricFixture.TYPE, allocator)
                    ...
                    var envelope = tech.streamfusion.flink.exchange.ArrowExchangeBatch.withEnvelope(
                            batch, SharedDeduplicateMetricFixture.TYPE)) {

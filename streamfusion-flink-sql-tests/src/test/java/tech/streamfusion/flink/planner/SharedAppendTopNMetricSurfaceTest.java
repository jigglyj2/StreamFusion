...
import static tech.streamfusion.flink.planner.SharedAppendTopNFixture.*;
...
class SharedAppendTopNMetricSurfaceTest {
            ...
            for (boolean rank : List.of(false, true))
                for (boolean before : List.of(false, true))
                    for (boolean rocks : List.of(false, true))
                        for (var range :
                                List.of(new int[] {1, 2}, new int[] {1, 10}, new int[] {2, 5}, new int[] {1, 64}))
                            modes.add(Arguments.of(ascending, rank, before, rocks, range[0], range[1]));
        return modes.stream();
    }

    @ParameterizedTest(name = "ascending={0}, rank={1}, before={2}, rocks={3}, range={4}..{5}")
    @MethodSource("modes")
    void sharedTreeMatchesFullDefaultStageMetricsAndOrderedFlinkChangelog(
            boolean ascending, boolean rank, boolean before, boolean rocks, int start, int end) throws Exception {
        var fixture = new SharedAppendTopNFixture(ascending, rank, before, start, end);

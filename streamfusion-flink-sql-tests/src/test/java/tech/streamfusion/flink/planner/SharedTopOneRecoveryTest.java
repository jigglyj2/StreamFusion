...
import static tech.streamfusion.flink.planner.SharedTopOneFixture.id;
...
class SharedTopOneRecoveryTest {
    ...
    void checkpointsBackendSwitchAndRescalingPreserveEveryTopOneTransition(boolean rocks, int mode, boolean ascending)
            throws Exception {
        for (int seed : List.of(3, 19, 71)) {
            var fixture = new SharedTopOneFixture(ascending, true, true);
    ...
    private static KeyedNativeMetricHarness region(
            SharedTopOneFixture fixture, boolean rocks, OperatorSubtaskState state, int parallelism, int subtask)
            throws Exception {
    ...
    private static byte[] exchange(SharedTopOneFixture fixture, int parallelism) {
        return NativeExchangePlanSerializer.hash(fixture.top.input, new int[] {0}, 16, parallelism, true);
    }

    private static void compare(
            SharedTopOneFixture fixture,

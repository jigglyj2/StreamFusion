...
/** Actual Flink barriers, persisted Arrow IPC channel state and native Top-1 state restored together. */
class SharedTopOneChannelRecoveryTest {
    ...
    void capturedWinnerUpdatesReplayExactlyOnce(boolean rocks, boolean unaligned) throws Exception {
        for (int seed : List.of(3, 19, 71)) {
            var fixture = new SharedTopOneFixture(false, true, true);
    ...
    private static byte[] exchange(SharedTopOneFixture fixture) {
    ...
    private static StreamTaskMailboxTestHarness<RowData> create(
            SharedTopOneFixture fixture, boolean rocks, boolean unaligned, TaskStateSnapshot state) throws Exception {
    ...
    private static byte[] bytes(SharedTopOneFixture fixture, StreamTaskMailboxTestHarness<RowData> task)
            throws Exception {

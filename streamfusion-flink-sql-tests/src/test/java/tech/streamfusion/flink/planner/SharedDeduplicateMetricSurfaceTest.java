...
import static tech.streamfusion.flink.planner.SharedDeduplicateMetricFixture.*;
...
class SharedDeduplicateMetricSurfaceTest {
    static Stream<Arguments> modes() {
        var modes = new ArrayList<Arguments>();
        for (boolean rowtime : List.of(false, true))
                ...
                for (boolean before : List.of(false, true))
                    for (boolean insert : List.of(false, true))
                        for (boolean rocks : List.of(false, true))
                            modes.add(Arguments.of(rowtime, last, before, insert, rocks));
        return modes.stream();
    }

    @ParameterizedTest(name = "rowtime={0}, last={1}, before={2}, insert={3}, rocks={4}")
    @MethodSource("modes")
    void sharedTreeMatchesFullDefaultStageMetricsAndOrderedFlinkChangelog(
            boolean rowtime, boolean last, boolean before, boolean insert, boolean rocks) throws Exception {
        var fixture = new SharedDeduplicateMetricFixture(rowtime, last, before, insert);
        try (var first = fixture.oracle(0, rocks);
                var middle = fixture.oracle(1, rocks);
                var tail = fixture.oracle(2, rocks);
                var target = new KeyedNativeMetricHarness(rocks, fixture.plan(), TYPE, List.of(id(1)));
                var allocator = new RootAllocator(64L << 20)) {
            var oracles = List.of(first, middle, tail);
            var expected = new DataOutputSerializer(128);
            compare(oracles, target, false);
            for (int arrival = 0; arrival < 4; arrival++) {
                int count = new int[] {0, 1, 7, 2049}[arrival];
                ...
                for (int row = 0; row < count; row++) {
                    var value = GenericRowData.of(
                            row % 7 == 0 ? null : StringData.fromString("é-" + row % 11),
                            TimestampData.fromEpochMillis(arrival * 10L + row % 4),
                    ...
                    var copy = new org.apache.flink.table.runtime.typeutils.RowDataSerializer(TYPE)
                            .toBinaryRow(value)
                            .copy();
                    feed(
                            oracles,
    ...
    private static void feed(List<FlinkStageMetricOracle> stages, StreamElement event, DataOutputSerializer output)
            throws Exception {

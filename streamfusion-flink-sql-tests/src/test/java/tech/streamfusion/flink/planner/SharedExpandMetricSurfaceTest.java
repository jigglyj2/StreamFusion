...
class SharedExpandMetricSurfaceTest {
    @Test
            ...
            try (var first = fixture.oracle(0);
                ...
                for (int arrival = 0; arrival < 4; arrival++) {
                    ...
                    for (int row = 0; row < count; row++) {
                        var value = GenericRowData.of(row % 7 == 0 ? null : row % 11 == 0 ? -1 : (row + seed) % 5);
                    ...
                    if (arrival == 3) {
                        assertThat(target.batches - previousBatches).isGreaterThanOrEqualTo(2);
                    }

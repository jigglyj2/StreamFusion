...
abstract class SharedStringScalarMetricFixture {
    ...
    final FlinkTypeFactory types = new FlinkTypeFactory(getClass().getClassLoader(), FlinkTypeSystem.INSTANCE);
    final RexBuilder rex = new RexBuilder(types);
    final RexNode id = rex.makeInputRef(types.createFieldTypeFromLogicalType(TYPE.getTypeAt(0)), 0);
    final List<RexNode> identity = List.of(id);

    abstract List<RexNode> projects(int stage);
    ...
    static String uid(int stage) {
        return "shared-string-metric-stage-" + stage;
    }

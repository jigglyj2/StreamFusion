...
/** Compares actual Flink-generated extraction, including the full internal millisecond range. */
class TimestampExtractRuntimeParityTest {
    private static final RowType INPUT = RowType.of(new TimestampType(3));

    @ParameterizedTest
    @ValueSource(booleans = {false, true})
    void generatedClockFieldsPreserveChangelogControlsAndMetrics(boolean rocks) throws Exception {
        ...
        for (var field :
                List.of(TimeUnitRange.HOUR, TimeUnitRange.MINUTE, TimeUnitRange.SECOND, TimeUnitRange.MILLISECOND)) {
            projections.add(rex.makeCall(SqlStdOperatorTable.EXTRACT, rex.makeFlag(field), timestamp));
        }

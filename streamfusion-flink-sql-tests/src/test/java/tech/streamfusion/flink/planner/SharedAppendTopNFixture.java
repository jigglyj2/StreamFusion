...
/** Same SQL-selected AppendOnlyTopN comparator in a Calc -> Top-N -> Calc native tree. */
final class SharedAppendTopNFixture {
    final AppendTopNFlinkFixture top;
    ...
    SharedAppendTopNFixture(boolean ascending, boolean rank, boolean before) throws Exception {
        this(ascending, rank, before, 1, 10);
    }

    SharedAppendTopNFixture(boolean ascending, boolean rank, boolean before, int start, int end) throws Exception {
        top = new AppendTopNFlinkFixture(ascending, rank, before, start, end);
    }
    ...
    static String name(int stage) {
        return "append-top-n-conformance-" + stage;
    }

    static String uid(int stage) {
        return "append-top-n-conformance-uid-" + stage;
    }
    ...
    FlinkStageMetricOracle oracle(int stage, boolean rocks) throws Exception {
        ...
        if (stage == 1) {
            ...
            var factory = CalcCodeGenerator.generateCalcOperator(
                    ...
                    true,
                    "AppendTopNConformanceCalc" + stage);

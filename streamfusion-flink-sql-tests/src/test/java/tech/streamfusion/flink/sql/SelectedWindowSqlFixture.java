...
/** Common generated source/configuration for selected window SQL graph parity. */
final class SelectedWindowSqlFixture {
    private SelectedWindowSqlFixture() {}

    static String counts(int windowSeconds) {
        return "SELECT k, COUNT(*) n, window_start s, window_end e "
                + "FROM TABLE(HOP(TABLE window_input, DESCRIPTOR(ts), INTERVAL '2' SECOND, INTERVAL '" + windowSeconds
                + "' SECOND)) "
                + "GROUP BY k, window_start, window_end";
    }

    static byte[] execute(boolean selected, boolean rocks, String sql, int seed, int parallelism) throws Exception {
        return execute(selected, rocks, sql, seed, parallelism, false);
    }

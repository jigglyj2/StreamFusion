...
class TimestampPartGeneratedParityTest extends SqlParityTestSupport {
    @ParameterizedTest
    @ValueSource(longs = {3, 19, 71})
    void clockFieldsPreserveEveryGeneratedChangelogAcrossFullTimestampRange(long seed) throws Exception {
    ...
    private static byte[] execute(List<Row> rows, boolean enabled) throws Exception {
        ...
        tables.createTemporaryView(
                ...
                tables.fromChangelogStream(
                        ...
                        Schema.newBuilder()
                                ...
                                .build(),
                        ChangelogMode.all()));
        String sql = "SELECT id, HOUR(ts), MINUTE(ts), SECOND(ts), "
                + "EXTRACT(MILLISECOND FROM ts), CAST(EXTRACT(HOUR FROM ts) AS INT), "
                + "CASE WHEN HOUR(ts) >= 8 AND HOUR(ts) <= 18 THEN 'dayTime' "
                + "WHEN HOUR(ts) <= 6 OR HOUR(ts) >= 20 THEN 'nightTime' ELSE 'otherTime' END "
                + "FROM extract_input WHERE id >= 0";
        ...
        converter.open(TimestampPartGeneratedParityTest.class.getClassLoader());

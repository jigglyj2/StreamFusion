...
class ArrayConstructorParityTest extends SqlParityTestSupport {
    @Test
    void arrayConstructorMatchesFlinkForScalarAndNestedArrayElements() throws Exception {
        assertDataStreamParity(
                "SELECT ARRAY[metric[1], 9, CAST(NULL AS INT)], "
                        + "ARRAY[metric, ARRAY_REVERSE(metric)] FROM array_input",
    ...
    @Test
    void arrayConstructorMatchesFlinkForNestedRowElements() throws Exception {
        assertDataStreamParity(
                "SELECT ARRAY[metric[1], metric[2], metric[1]] FROM row_array_input",

...
class SetOperationsFallbackParityTest extends SqlParityTestSupport {
    ...
    void intersectAllReplicatesNestedArrowValues() throws Exception {
        ...
        SqlFallbackAssertions.nativeBatchesAreZero(
                tech.streamfusion.flink.StreamFusionPlannerFactory.nativePlanBatchCount());
    }
    ...
    void setOperationMatchesFlinkForNulls(String operation) throws Exception {
        ...
        SqlFallbackAssertions.admission();
        SqlFallbackAssertions.nativeBatchesAreZero(
                tech.streamfusion.flink.StreamFusionPlannerFactory.nativeGroupAggregateBatchCount());
        if (operation.endsWith("ALL")) {
            SqlFallbackAssertions.nativeBatchesAreZero(
                    tech.streamfusion.flink.StreamFusionPlannerFactory.nativePlanBatchCount());
        } else {
            SqlFallbackAssertions.nativeBatchesAreZero(
                    tech.streamfusion.flink.StreamFusionPlannerFactory.nativeRegularJoinBatchCount());
        }
    }

    static java.util.stream.Stream<Arguments> setOperationTypes() {
        return SelectDistinctFallbackParityTest.distinctTypes();
    }
}

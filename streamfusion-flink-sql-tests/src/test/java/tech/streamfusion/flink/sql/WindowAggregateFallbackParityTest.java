...
class WindowAggregateFallbackParityTest extends SqlParityTestSupport {
    ...
    @org.junit.jupiter.api.Test
    void twoPhaseWindowPlanReportsBothBlockedStagesAndKeepsFlinkExchange() {
        System.setProperty(
                StreamFusionPlannerFactory.FACTORY_CLASS_PROPERTY, StreamFusionPlannerFactory.class.getName());
        StreamTableEnvironment tables = StreamTableEnvironment.create(
                StreamExecutionEnvironment.getExecutionEnvironment(),
        ...
        assertThat(plan)
                .contains("Accelerated: no", "StreamExecLocalWindowAggregate", "StreamExecGlobalWindowAggregate")
                .doesNotContain(
    ...
    void alignedWindowCountsAndPropertiesMatchFlinkByteForByte(String windowCall) throws Exception {
        ...
        if (!windowCall.startsWith("SESSION")) {
            SqlFallbackAssertions.nativeBatchesAreZero(
                    StreamFusionPlannerFactory.nativeLocalWindowAggregateBatchCount());
            SqlFallbackAssertions.nativeBatchesAreZero(StreamFusionPlannerFactory.nativeWindowAggregateBatchCount());
        }
    }
    ...
    void aggregateOverAttachedWindowColumnsMatchesFlinkByteForByte() throws Exception {
        ...
        assertThat(streamFusion).isEqualTo(flink);
        SqlFallbackAssertions.nativeBatchesAreZero(StreamFusionPlannerFactory.nativeLocalWindowAggregateBatchCount());
        SqlFallbackAssertions.nativeBatchesAreZero(StreamFusionPlannerFactory.nativeWindowAggregateBatchCount());
        SqlFallbackAssertions.admission();
    }

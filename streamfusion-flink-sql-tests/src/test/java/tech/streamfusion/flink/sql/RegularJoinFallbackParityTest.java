...
class RegularJoinFallbackParityTest extends SqlParityTestSupport {
    @Test
    ...
    @Test
    void residualConditionFiltersInnerJoinCandidatesWithFallback() throws Exception {
        String query = "SELECT l.id, l.payload, l.attributes, r.payload, r.amount "
    ...
    @Test
    void residualConditionControlsSemiJoinMembershipWithFallback() throws Exception {
        String query = "SELECT l.id, l.payload, l.attributes FROM regular_join_left l WHERE "
                + "EXISTS (SELECT 1 FROM regular_join_right r WHERE l.id = r.id "

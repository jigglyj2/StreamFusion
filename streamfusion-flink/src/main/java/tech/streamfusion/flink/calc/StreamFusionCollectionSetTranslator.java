...
final class StreamFusionCollectionSetTranslator extends StreamFusionComplexTypeSupport {
    ...
    static Expression arrayUnion(Object expression, RowType inputType, LogicalType expectedType) {
        if (!"ARRAY_UNION".equals(functionName(expression)) || !(expectedType instanceof ArrayType)) {
            return null;
        }
        java.util.List<?> operands = (java.util.List<?>) invoke(expression, "getOperands");
        if (operands.size() != 2) {
        ...
        return left == null || right == null
                ? null
                : Expression.newBuilder()
                        .setArrayUnion(ArrayUnion.newBuilder().setLeft(left).setRight(right))
                        .build();
    }

    static Expression arrayIntersect(Object expression, RowType inputType, LogicalType expectedType) {
        if (!"ARRAY_INTERSECT".equals(functionName(expression)) || !(expectedType instanceof ArrayType)) {
            return null;
        }
        java.util.List<?> operands = (java.util.List<?>) invoke(expression, "getOperands");
        if (operands.size() != 2) {

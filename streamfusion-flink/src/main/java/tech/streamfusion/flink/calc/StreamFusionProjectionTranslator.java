...
abstract class StreamFusionProjectionTranslator extends StreamFusionRexSupport {
    protected static Expression projectionExpression(
        ...
        Expression inetAton = StreamFusionInetFunctionTranslator.aton(expression, inputType, expectedType);
        if (inetAton != null) {
            return inetAton;
        }
        ...
        Expression identityCast = StreamFusionIdentityCastTranslator.translate(expression, inputType, expectedType);
        ...
        Expression unhex = StreamFusionBinaryFunctionTranslator.unhex(expression, inputType, expectedType);
        if (unhex != null) {
            return unhex;
        }
        ...
        Expression stringElt = StreamFusionStringFunctionTranslator.elt(expression, inputType, expectedType);
        if (stringElt != null) {
            return stringElt;
        }
        ...
        Expression jsonFunction = StreamFusionJsonFunctionTranslator.translate(expression, inputType, expectedType);

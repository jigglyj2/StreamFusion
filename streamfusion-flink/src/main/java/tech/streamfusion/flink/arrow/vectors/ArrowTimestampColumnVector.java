...
package tech.streamfusion.flink.arrow.vectors;
    ...
    public TimestampData getTimestamp(int i, int precision) {
        if (valueVector instanceof TimeStampSecVector) {
            ...
            return TimestampData.fromEpochMillis(
                    Math.floorDiv(micros, 1000), (int) Math.floorMod(micros, 1000L) * 1000);
        } else {
            long nanos = ((TimeStampNanoVector) valueVector).get(i);
            return TimestampData.fromEpochMillis(
                    Math.floorDiv(nanos, 1_000_000), (int) Math.floorMod(nanos, 1_000_000L));
        }
    }

...
public final class ArrowMatchRecognizeCDataBridge {
    ...
    private static ArrowRowDataBatch importOutput(
        ...
        if (count < 0 || count > Integer.MAX_VALUE) {
            throw new IllegalStateException("Native match recognize returned invalid row count " + count);
        }
        ...
        if (rowKindIndex < 0 || !(output.getVector(rowKindIndex) instanceof TinyIntVector)) {
            output.close();
            throw new IllegalStateException("Native match recognize returned invalid RowKind metadata");
        }

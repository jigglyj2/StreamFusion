...
class ArrowExchangeBatchTest {
    ...
    void transposesValuesAndTheCompleteFlinkRecordEnvelope() {
        ...
        List<StreamRecord<RowData>> records = List.of(new StreamRecord<>(insert, 123L), new StreamRecord<>(delete));

        try (RootAllocator allocator = new RootAllocator(Long.MAX_VALUE);
                ArrowRowDataBatch batch = ArrowExchangeBatch.transpose(records, RowType.of(new IntType()), allocator)) {
